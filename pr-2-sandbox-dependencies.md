...
repointed), across this repo and `platform-packages`:

- **50 of the 59 published tags have carried more than one artifact**, for a total
  of **201 silent replacements**.
- `ingress-nginx:4.12.1-p03` has served **six different digests** — and it is the
  only tag that package has ever had. Every change to it since 4 Aug went out under
  the same reference.
- `tools:1.0.0-p01` and `cloudnative-pg:1.29.1-p01` have six digests each;
  `keycloak:24.4.11-p14` and `seaweedfs:4.17.0-p08` have four.

The overwrite is not always a forgotten bump. Every merge rebuilds every manifest
under `packages/` — there is no changed-file filter — and the build pulls its
module charts at build time, so it is not reproducible. A package nobody edited
still produces a new digest and still gets pushed over its own tag.

A cluster that reconciles after the fact pulls different content under a reference
it had already resolved. `OKDP/okdp-sandbox#87` is one instance of the class:
`local-secrets-provider` moved under the team and `cnpg-postgresql` failed on a
secret that no longer existed.
...
**3. One broken package no longer stops the other thirteen.** The loop collects
failures and reports them together instead of aborting on the first one. That was
the second half of #23.
...
for package in ${KUBOCD_PACKAGES}; do
  ...
  n=$(curl -s "https://quay.io/api/v1/repository/okdp/sandbox-dependencies%2F${name}/tag/?specificTag=${tag}&onlyActiveTags=true" \
      | jq '.tags | length')
  [ "$n" -gt 0 ] && echo "SKIP  $name:$tag" || echo "BUILD $name:$tag"
done
```

All fourteen print `SKIP` today. Before this PR, dispatching `publish.yml` would
have re-pushed every one of them over itself.

**2. Only a bumped package is published.** Bump one `tag:`, dispatch `publish.yml`,
and check the job summary: a "Package publication plan" table listing thirteen
`skipped, already published` and one `build and push`.

**3. A pull request still gets a full build.** Open any PR — `ci` builds all
fourteen packages to `ghcr.io` exactly as before. The guard does not apply there.
...
```yaml
      oci_package_prefix:
        description: "OCI package prefix, e.g., 'quay.io/my-org/<PACKAGE-PREFIX>', i.e.: <PACKAGE-PREFIX> = sandbox-dependencies"
        required: true
        type: string
